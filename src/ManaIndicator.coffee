...
#        ("<td><h1>#{val - (if @responsive then (@player.mana_active[key] + @player.mana_used[key]) else 0)}</h1></td>" for key, val of @player.mana).join('\n')
        ("<td><h1>#{val - (if @responsive then (@player.mana_used[key]) else 0)}</h1></td>" for key, val of @player.mana).join('\n')
      )

      @mana[key] = @player.mana[key] - (if @responsive then (@player.mana_used[key]) else 0) for key of @player.mana
  ...
  is_dirty: () ->
    ...
    (1 for key of @player.mana when @player.mana[key] - @player.mana_used[key] isnt @mana[key]).length > 0 or (1 for key of @mana when @player.mana[key] - @player.mana_used[key] isnt @mana[key]).length > 0
